...
TODO: ERIC JEFFERS AND MARCUS DEL RIO, 11/2014
...
void train()
{
	...
	for (int q=0;q<diffsamplecount;q++)
	{
		while(true)
		{
			trained=true;//flag boolean for if the training is finished for each sample input	
	...
	ofstream datafile3;
	//this is where weights are written to perceptron.txt
	datafile3.open("supinfo.txt",ios::out|ios::trunc);// (ios::trunc overwrites current file, ios::app will simply add to existing file
	datafile3<<diffsamplecount<<endl;
	for (int w=0;w<diffsamplecount;w++)
		datafile3<<foundlist[w]<<endl;
	datafile3 << endl;
	datafile3.close();
...
void test()
{
	...
	ifstream datafile3;	//file object
	string line3;		//lines will go here
	datafile3.open("supinfo.txt");	//open the file
	
	datafile3>>diffsamplecount;
	char foundlist[diffsamplecount];
	for (int i=0;i<diffsamplecount;i++)
		datafile3>>foundlist[i];
	...
	string letterpred= " "; //store the letter that matches up with the highweight
	for (int v=0;v<diffsamplecount;v++)
	{
		predictionSquares=getSquares();
		prediction=brain[v]->getRawPrediction(predictionSquares);
		if (prediction > highweight) //find the highest weight comparatively and store it
		{
			highweight = prediction;
			letterpred = foundlist[v];
		}
	}
	cout<<"my prediction is "<<letterpred<< "!" <<endl;
